...
Game.resetMapObjects = function () {
    ...
    for (var y = 0; y < GRID_HEIGHT; y++) {
        for (var x = 0; x < GRID_WIDTH; x++) {
            ...
            } else if (tile == "@") { // register body incrementing pills (pac-nibble)                
                this.objects.set(x, y, {
                    type: Objects.NIBBLE,
                    collected: false
                });
...
Game.drawPills = function (ctx) {
    ...
    for (var x = 0; x < GRID_WIDTH; x++) {
        for (var y = 0; y < GRID_HEIGHT; y++) {
            ...
            } else if (this.checkNibble(x, y)) {
                this.drawNibble(x, y, ctx)
            }
        }
    }
}
...
Game.checkNibble = function (x, y) {
    return this.objects.get(x, y)
        && this.objects.get(x, y).type == Objects.NIBBLE
        && this.objects.get(x, y).collected == false
}
